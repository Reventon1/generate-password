...
from rich import print, box, table
...
print_count_array_symbols = Text.from_markup(f'{len(password.get_array_symbols())}', style="bold green")
table_info.add_row(f'Количество доступных символов:', print_count_array_symbols)

print_array_symbols = Text.from_markup(f'{password.get_array_symbols()}', style="bold green")
table_info.add_row(f'Доступные символы:', print_array_symbols)

print_count_variant = Text.from_markup(f'{password.count_variant}', style="bold green")
table_info.add_row(f'Количество возможных комбинаций: ', print_count_variant)

print_password = Text.from_markup(f'{password.password}', style="bold green")
table_info.add_row(f'Сгенерированный пароль: ', print_password)

print_pin_code = Text.from_markup(f'{password.pin_code}', style="bold green")
table_info.add_row(f'Пин код: ', print_pin_code)

print_check_summa = Text.from_markup(f'{password.check_summa}', style="bold green")
...
layout.update(
    Panel(
        ...
        title="Информация",
        subtitle="Приложение версии 0.0.8",
